...
Last updated: 2026-05-13 (Phase 5 complete + Phase 6 hierarchical planning shipped)
...
**You are here:** Phases 0–6 all shipped. Phase 5 closed out the self-correcting executor (stuck detection, action/completion verification, jitter retries, bounds-aware validation, run analytics, timelines, latency profiles, SeeClick ingestion). Phase 6 added the hierarchical planning layer: an opt-in text-only MLX-LM (Qwen2.5-7B-Instruct-4bit by default) decomposes the user goal into ≤6 subgoals, each of which the vision agent executes; subgoal failures trigger a replan against the current state. A fresh clone goes from `git clone` to a working install via `uv sync && nalu onboard`; the planner LLM is an *optional* onboarding step so vision-only installs still ship.

**Major outstanding work item:** Phases 3, 4, 5, and 6 are uncommitted. `git log` reaches only `aa6e86f trainer: eval comparison panel — Phase 2 complete` — ~50 modified/untracked files (menubar, onboarding, wakeword, registry, mergekit, planner_llm, all Phase 5 self-correction modules + tests, README/CHANGELOG/CONTRIBUTING, `uv.lock`) sit in the working tree. Single biggest exposure on the project.

**Next pull (post-commit):** real-user feedback loop. Watch what people hit on first run, what tasks they actually try, and what the planner+executor get wrong. Likely Phase 7 candidates: signed-binary distribution if packaging is real friction, custom "hey nalu" wake-word model (currently `hey_jarvis` placeholder), curated dataset shipped with releases so the LoRA pipeline is useful day-one, and a subgoal-level eval harness that mirrors the existing per-step `nalu train eval` but scores plans end-to-end.
